...
// MongoDB Configuration for Permanent Data Storage (set MONGODB_URI in Render env)
...
// Clear data only when explicitly requested (avoids wiping data on every Render restart)
function isManualDeployment() {
  if (process.env.CLEAR_DATA_ON_DEPLOY === "true") {
    console.log("🔍 Data clear requested via CLEAR_DATA_ON_DEPLOY=true");
    return true;
  }
  console.log("🔍 Automatic restart — data preserved (set CLEAR_DATA_ON_DEPLOY=true to reset on deploy)");
  return false;
}
